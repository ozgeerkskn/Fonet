...
export const addItemToCart = (reqData) => {
  return async (dispatch) => {
    ...
    try {
      const { data } = await api.put(`/api/card/add`, reqData.cardItem, {
        headers: {
          Authorization: `Bearer  ${reqData.token}`,
        },
      });
      console.log("Add item to card.", data);
      dispatch({ type: ADD_ITEM_TO_CARD_SUCCESS, payload: data });
    } catch (error) {
      console.log("catch error", error);
      dispatch({ type: ADD_ITEM_TO_CARD_FAILURE, payload: error.message });
    }
  };
};
...
export const updateCartItem = (reqData) => {
  return async (dispatch) => {
    ...
    try {
      ...
      console.log("Update cardItem", data);
...
export const removeCartItem = ({ cardItemId, jwt }) => {
  return async (dispatch) => {
    dispatch({ type: REMOVE_CARTITEM_REQUEST });
    try {
      const { data } = await api.delete(`/api/card-item/${cardItemId}/remove`, {
        headers: {
          Authorization: `Bearer  ${jwt}`,
        },
      });
      console.log("Remove cardItem", data);
      dispatch({ type: REMOVE_CARTITEM_SUCCESS, payload: cardItemId });
    } catch (error) {
      console.log("catch error", error);
